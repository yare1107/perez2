...
const handler20vs20 = async (m, { conn, args }) => {
    if (args.length < 2) {
...
handler20vs20.help = ['20vs20'];
handler20vs20.tags = ['freefire'];
handler20vs20.command = /^(20vs20|vs20)$/i;

// Sistema de reacciones para 20vs20
handler20vs20.before = async function (m) {
    ...
    if (!partidasVS20[key.id]) return false;
...
${data.jugadores.length < 20 || data.suplentes.length < 2 ? '(𝚁𝚎𝚊𝚌𝚌𝚒𝚘𝚗𝚊 𝚌𝚘𝚗 ❤️ 𝚙𝚊𝚛𝚊 𝚞𝚗𝚒𝚛𝚝𝚎)' : '✅ 𝐋𝐈𝐒𝐓𝐀 𝐂𝐎𝐌𝐏𝐋𝐄𝐓𝐀'}
    `.trim();
